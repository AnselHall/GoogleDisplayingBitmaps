...
public class ImageCache {
    ...
    // Default memory cache size in kilobytes  默认内存缓存大小，单位：千字节  1024 字节，KB
    private static final int DEFAULT_MEM_CACHE_SIZE = 1024 * 5; // 5MB

    // Default disk cache size in bytes   默认磁盘缓存大小，单位：字节
    private static final int DEFAULT_DISK_CACHE_SIZE = 1024 * 1024 * 10; // 10MB

    // Compression settings when writing images to disk cache
    //在磁盘缓存中图片的压缩格式，默认 JPEG
    ...
    //线程安全的集合 通过 Collections.synchronizedSet(new HashSet<SoftReference<Bitmap>>()) 获得
    ...
    public static File getDiskCacheDir(Context context, String uniqueName) {
        ...
        //检查外部存储是否挂载可用，如果可用就使用外部缓存路径，否则，会使用内部缓存路径 context.getCacheDir().getPath();
        final String cachePath =
                Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState()) ||
                        !isExternalStorageRemovable() ? getExternalCacheDir(context).getPath() :
                                context.getCacheDir().getPath();

        //DiskCacheDir: /storage/10FD-2C17/Android/data/com.example.android.displayingbitmaps/cache/thumbs
        //DiskCacheDir: /storage/10FD-2C17/Android/data/com.example.android.displayingbitmaps/cache/http
    ...
    public static File getExternalCacheDir(Context context) {
        if (Utils.hasFroyo()) {
            //在 Android 2.2 (Froyo)之后，可以直接通过 context.getExternalCacheDir() 获得缓存路径
            //缓存路径：/storage/10FD-2C17/Android/data/com.example.android.displayingbitmaps/cache
//            android.util.Log.e(TAG, "getExternalCacheDir: " + context.getExternalCacheDir() );
            return context.getExternalCacheDir();
        }

        // Before Froyo we need to construct the external cache dir ourselves
        //在 Froyo 之前，需要手动拼接缓存路径
        final String cacheDir = "/Android/data/" + context.getPackageName() + "/cache/";
        //  /storage/10FD-2C17/Android/data/com.example.android.displayingbitmaps/cache
